...
Scenario: Validate Extraction Rule input form by not entering any data
	...
	| Type      |
	| Data Type |
	And the User updates the Add Extraction Rule field 'PartToMask' with values 'Please select'
...
Scenario: Validate name field character length cannot exceed 50 characters
	And the User updates the Add Extraction Rule field 'Name' with the text 'Lorem ipsum dolor sit amet, consectetur adipiscinga'	
    When I save the Extraction Rule
	Then I should get an error message for the following fields:
	| Field | Error Text                 |
...
Scenario: Validate that you cannot add an Extraction Rule that already exists
	And the User updates the Add Extraction Rule field 'SearchName' with the text '_Animals'
    When I save the Extraction Rule
	Then I should get an error message for the following fields:
	| Field | Error Text							|
